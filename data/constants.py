...
POPULAR_BOOK_SQL = "SELECT loans.bookCopiesID, book.title, book.genre, authors.authorName, COUNT(loans.checkoutDate) from loans INNER JOIN bookCopies ON loans.bookCopiesID = bookCopies.id INNER JOIN book ON bookCopies.bookID = book.id INNER JOIN authors ON book.authorID = authors.id GROUP BY loans.bookCopiesID ORDER BY COUNT(loans.checkoutDate) DESC"
...
BOOK_TITLE_SEARCH_SQL = "SELECT book.title FROM bookCopies JOIN book ON bookCopies.bookid = book.id JOIN authors ON book.authorid = authors.id WHERE book.title LIKE ?"
BOOK_AUTHOR_SEARCH_SQL = "SELECT book.title FROM bookCopies JOIN book ON bookCopies.bookid = book.id JOIN authors ON book.authorid = authors.id WHERE authors.authorname LIKE ?" 
BOOK_TITLE_AUTHOR_SEARCH_SQL = "SELECT book.title FROM bookCopies JOIN book ON bookCopies.bookid = book.id JOIN authors ON book.authorid = authors.id WHERE book.title LIKE ? AND authors.authorname LIKE ?"
